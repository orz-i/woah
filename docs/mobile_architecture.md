...
2. **V1 稳定性约束**：
   ...
   - Full Body 的人物选择、Effect Preview 与最终 Export 统一使用 tight mask：YOLO 原始 Mask 进入 `TrackManager`，渲染时使用 0.5 阈值的 canonical 轮廓并跳过额外 dilation，避免不同阶段的人体轮廓大小发生变化；FACE_ONLY 仍保持独立隐私贴纸/回退策略。
