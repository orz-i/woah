...
class PreviewPipeline(
    ...
    suspend fun renderPreview(request: PreviewRequestDto): PreviewFrameDto = withContext(Dispatchers.Default) {
        ...
        val segResult = segmenter.segmentBitmapSync(rotatedBitmap, request.timestampMs * 1000L)
        ...
        val trackedPersons = if (metadata != null && metadata.persons.isNotEmpty() && detections.isNotEmpty()) {
            ...
            val costMatrix = Array(cached.size) { r ->
                ...
                FloatArray(detections.size) { c ->
                    ...
                    val dx = (cBox.centerX - dBox.centerX) / frameWidth.toFloat()
                    val dy = (cBox.centerY - dBox.centerY) / frameHeight.toFloat()
        ...
        val previewWidth = minOf(frameWidth, 1280)
        val previewHeight = (previewWidth * (frameHeight.toFloat() / frameWidth)).toInt().coerceAtLeast(1)
