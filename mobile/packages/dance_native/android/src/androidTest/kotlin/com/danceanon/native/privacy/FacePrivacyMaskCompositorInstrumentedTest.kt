...
/**
 * Proves that a face-only NativeMask can travel through the unchanged production
 * privacy sampler/compositor. No face-only runtime call site is added by this test.
 */
@RunWith(AndroidJUnit4::class)
class FacePrivacyMaskCompositorInstrumentedTest {
    @Test
    fun faceMaskUsesExistingPrivacySamplerWithoutVerticalFlip() {
        val egl = EglCore()
        ...
        try {
            ...
            val faceMask = assertNotNull(FacePrivacyMaskBuilder.build(regions, mapper))
            val person = TrackedPerson(
                id = 7,
                bbox = FloatRect(100f, 20f, 540f, 350f),
                mask = faceMask,
                confidence = 0.95f,
            ...
            val rendered = assertNotNull(renderer.captureRenderedFrame())
            try {
                assertRed(rendered, 190, 78, "detected face center")
                assertRed(rendered, 455, 100, "fallback head center")

                // If mask Y were inverted, these mirrored lower-body points would
                // be red. They must remain the untouched white source instead.
                assertWhite(rendered, 190, FRAME_H - 78, "detected mirror")
                assertWhite(rendered, 455, FRAME_H - 100, "fallback mirror")
                assertWhite(rendered, 320, 300, "clear lower body")
            } finally {
                rendered.recycle()
            }
        } finally {
