...
- 选定主角后，编辑器的 9:16 裁切预览可轻触播放/暂停，并按当前播放时间约 4 fps 节流刷新 native 效果帧；最终导出仍按源视频逐帧 PTS 执行完整运镜。
...
9:16 动态预览和最终导出都使用同一后处理结构与 18×32 整数单位画幅；预览请求按时间点异步失效旧结果，预览失败时不会随意换主角。
...
iOS 动态时间点预览也复用与 Android 一致的精确 9:16 尺寸规则。
