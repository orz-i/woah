...
仅实现 `mobile`；不修改历史 `desktop`。入口位于统一保护编辑器的“画面裁切”。
...
9:16 导出使用宽高分别为 18、32 的整数倍，保证编码偶数尺寸、精确比例及常规源尺寸不放大，最长边不超过 1920。
例如 1920×1080 源输出 594×1056，3840×2160 源输出 1080×1920；不是将横屏强行拉伸成 1080×1920。
